...
module.exports = class Crawler {
  ...
  async _process() {
    ...
    console.log(`${pluralize(failed.length, "post")} had failed downloads:`);
    console.log(failed);
    
    const destDir = settings.get("download_dir");
    const blogDir = path.join(destDir, this.blog);
    const compact = settings.get("compact_json");
    const meta = JSON.stringify({failures: failed}, null, compact ? null : 2);
    fs.writeFileSync(path.join(blogDir, "meta.json"), meta);
