...
module.exports = function(post) {
  ...
  if (!overwrite && fs.existsSync(metaFile)) {
    ...
    if (meta.failures.length === 0) {
      ...
      log.debug(`Post ${post.meta.id} was already downloaded, but had ${meta.failures.length}. Retrying...`);
    }
  }
  
  log.trace(`Downloading post ${post.meta.id} from ${blogName}`);
  log.debug(`Downloading ${post.meta.files.length} files from post ${post.meta.id}`);
  
  return downloadAll(blogDir, post.meta.files)
    ...
    .tap((failures) => {
      const postDir = path.join(blogDir, "posts");
      const jsonFile = path.join(postDir, `${post.meta.id}.meta.json`);
      ...
      return fs
        .ensureDir(postDir)
        .then(() => fs.writeFile(jsonFile, JSON.stringify(post.meta, null, compact ? null : 2)))
      ;
    })
